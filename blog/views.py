...
import markdown#语法高亮
import pygments#目录
from django.db.models import Q  #搜索
from django.core.paginator import Paginator,EmptyPage,PageNotAnInteger #博客分页
from django_comments import models as comment_models

# Create your views here.

def make_paginator(objects,page,num=5):##分页器
    paginator = Paginator(objects,num)
    try:
        object_list = paginator.page(page)
    except PageNotAnInteger:#防止页码输入有错破坏系统
        object_list = paginator.page(1)
    except EmptyPage:
        object_list = paginator.page(paginator.num_pages)#若不输入页码 则返回最后一页
    return object_list,paginator

def pagination_data(paginator, page):#自定义分类器
    """
    用于自定义展示分页页码的方法
...
def detail(request,blog_id):
    ...
    def get_comment_list(comments):
        for comment in comments:
            ...
            if len(children) > 0:
                get_comment_list(children)#递归调用
...
def category(request,category_id):  #分类视图
    # c = models.Category.objects.get(id=category_id)
    c = get_object_or_404(models.Category,id=category_id) #防止输入出错
...
def tag(request,tag_id):
    ...
    if t.name == '全部':
        entries = models.Entry.objects.all()  #标签全部是显示所有标签
    else:
...
def search(request):
    ...
    entries = models.Entry.objects.filter(Q(title__icontains=keyword)
                                          ...
                                          |Q(abstract__icontains=keyword))  #过滤函数
    # else:
    #     entries = '无相关内容'
...
def login(request):
    ...
    access_token_url = 'https://api.weibo.com/oauth2/access_token?client_id=%s&client_secret=%s&grant_type=authorization_code&redirect_uri=http://www.xyfei.cn/login&code=%s'\
